...
export class DraftQueueRepository implements IDraftQueueRepository {
  ...
  async getQueueForRoster(draftId: number, rosterId: number): Promise<DraftQueueWithPlayer[]> {
    ...
    return result.rows.map(row => {
      ...
      const player = row.player_id ? Player.fromDatabase(row) : undefined;
      return { ...queue, player };
    });
  }

  async addToQueue(draftId: number, rosterId: number, playerId: number): Promise<DraftQueue> {
    ...
    return DraftQueue.fromDatabase(result.rows[0]);
  }
